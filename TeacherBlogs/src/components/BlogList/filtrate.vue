<template>
  <ul class="blog-list-filtrate">
    <li>
      ...
      <span class="li-choose-one" v-for="(item,index) in choose_condition" :class="{active:item.active}" @click="tab_change(index,item.tit)">{{item.tit}}</span>
    </li>
    <li>
      <strong>二级分类：</strong>
      <span class="type-all" @click="sec_all">全部</span>
      <span @click="sec_tab(it.name)" :class="{active:it.active}" class="li-choose-one" v-for="(it,ind) in arr1">{{it.name}}</span>
    </li>
    <li v-show="arr2.length !== 0 ">
      <span @click="sec_tab(it.name)" :class="{active:it.active}"  class="li-choose-one" v-for="(it,ind) in arr2">{{it.name}}</span>
    </li>
  </ul>
</template>

<template>
  <ul class="class-condition">
      <li>
        ...
        <span class="li-choose" :class="{active:item.type}" v-for="(item,index) in class_list" @click="choose_class(item.con,index)">{{item.con}}</span>
      </li>
    <li>
      <span class="li-tit">优惠活动：</span>
      <span class="li-choose-all" @click="choose_discounts('全部')" :class="{active:all_discounts_type}">全部</span>
      <span class="li-choose" :class="{active:item.type}" v-for="(item,index) in discounts_list" @click="choose_discounts(item.con,index)">{{item.con}}</span>
    </li>
    <li>
      <span class="li-tit">培训费用：</span>
      <span class="li-choose-all" @click="choose_price('全部')" :class="{active:all_price_type}">全部</span>
      <span class="li-choose" :class="{active:item.type}" v-for="(item,index) in price_list" @click="choose_price(item.con,index)">{{item.con}}</span>
    </li>
  </ul>
</template>
